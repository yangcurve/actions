...
type Resolver<Context, Input, Output> = (param: {
    ctx: Context;
    input: Input;
}) => Promise<Output>;
type ActionBuilder<Type extends ActionType, Context, Schema extends z.ZodType> = <Output>(resolver: Resolver<Context, z.infer<Schema>, Output>) => Action<Type, z.input<Schema>, Output>;
type ActionBuilderWithoutInput<Type extends ActionType, Context> = ActionBuilder<Type, Context, z.ZodVoid>;
type ActionBuilderWithInput<Context> = <Schema extends z.ZodType>(schema: Schema) => {
...
type ProcedureBuilder = <Context>(createContext?: () => Promise<Context>) => Procedure<Context>;
...
declare const createClientApi: <As extends Record<string, unknown>>(actions: As) => {
    query: <Key extends FlattenObjectKeysWithFilter<"query", As>>(actionKey: Key) => (input: InferActionIOFromAction<"input", InferActionFromKey<Key, As>>, options?: Omit<Omit<UndefinedInitialDataOptions<InferActionIOFromAction<"output", InferActionFromKey<Key, As>>>, "queryFn">, "queryKey">) => _tanstack_react_query.UseQueryResult<InferActionIOFromAction<"output", InferActionFromKey<Key, As>>, Error>;
    mutation: <Key extends FlattenObjectKeysWithFilter<"mutation", As>>(actionKey: Key) => (options?: Omit<Omit<UseMutationOptions<InferActionIOFromAction<"output", InferActionFromKey<Key, As>>, Error, InferActionIOFromAction<"input", InferActionFromKey<Key, As>>, unknown>, "mutationKey">, "mutationFn">) => _tanstack_react_query.UseMutationResult<InferActionIOFromAction<"output", InferActionFromKey<Key, As>>, Error, InferActionIOFromAction<"input", InferActionFromKey<Key, As>>, unknown>;
};
